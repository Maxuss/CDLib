...
import space.maxus.xai.save.fields.*
...
object TypeSerializer {
    ...
    fun <T: Any> deserialize(data: ByteArrayInputStream, klass: KClass<T>): T {
        if(data.read() != 0x09)
            throw SerializationException("Invalid file data provided!")
        if(klass.isAbstract || klass.java.isInterface)
            throw SerializationException("Could not deserialize data! The provided type of ${klass.qualifiedName} can not be instantiated!")
        ...
        if(klass.hasAnnotation<RepresentedBy>()) {
            ...
            val members = klass.declaredMemberProperties.filter { f -> reprs.contains(f.name) }.filterIsInstance<KMutableProperty<*>>()

            for (member in members) {
                member.javaSetter?.invoke(instance, FieldType.findByInt(data.read()).serializer.deserialize(data)?.value)
            }
        ...
        if(fields.size < size)
            throw SerializationException("Invalid amount of fields for serialization provided!")
